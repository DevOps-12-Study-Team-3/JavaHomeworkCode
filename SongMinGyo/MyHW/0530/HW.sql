...
CREATE TABLE SCORE_CHK(
	...
	CNO VARCHAR2(8),
	RESULT NUMBER(3, 0),
	CONSTRAINT RESULT CHECK(RESULT BETWEEN 60 AND 90)
);
...
--CNO, CNAME을 PK로 PNO은 PROFESSOR_PK의 PNO을 참조하여 FK로 설정하고 
...
DECLARE 
	U_CNO COURSE.CNO%TYPE;
	U_CNAME COURSE.CNAME%TYPE;
	U_PNO PROFESSOR.PNO%TYPE;
	U_PNAME PROFESSOR.PNAME%TYPE;

BEGIN
	SELECT C.CNO, C.CNAME, P.PNO, P.PNAME INTO U_CNO, U_CNAME, U_PNO, U_PNAME
		FROM COURSE C
		JOIN PROFESSOR P
	      ON P.PNO = C.PNO
	    WHERE C.CNAME = '유기화학';
	DBMS_OUTPUT.PUT_LINE(U_CNO);
	DBMS_OUTPUT.PUT_LINE(U_CNAME);
	DBMS_OUTPUT.PUT_LINE(U_PNO);
	DBMS_OUTPUT.PUT_LINE(U_PNAME);
END;
